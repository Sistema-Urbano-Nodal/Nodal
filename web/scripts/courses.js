...
let conversations=[],pageActive=true;
...
async function course(){
  stopConversations();navigationSequence++;navigationPending=false;snapshot=await api(base());root.replaceChildren();
  ...
  if(!snapshot.enrollment&&!snapshot.isAdmin){
    const enroll=button('enroll',async()=>{enroll.disabled=true;try{await api(base()+'/enroll',{});await course();}catch(err){status(msg,err);enroll.disabled=false;}});
...
async function openModule(id){
  const navigation=++navigationSequence,content=document.getElementById('moduleContent');
  ...
  try{
    ...
    navigationPending=false;stopConversations();activeId=id;const sequence=++loadSequence;
    ...
    content.replaceChildren();const head=el('section','pilot-module-heading');
  ...
  }catch(err){if(navigation===navigationSequence){navigationPending=false;conversations.forEach(view=>view.resume());status(msg,err);}}
}
