...
- Flavia Muro Doig and Mariana Alegre were assigned the server-managed NODAL admin role after matching confirmed account identities. The user explicitly clarified Mariana Alegre as the intended Mariana. No personal passwords were reset.
- Password recovery now uses Supabase PKCE, a signed HttpOnly verifier cookie, a fixed allowlisted callback and a provider-issued recovery session. Tests cover expired/tampered cookies, proof ownership/method, consumed codes, request throttling, cookie renewal and uncertain network outcomes. The production callback and existing custom SMTP were verified configured.
- A disposable real-provider test verified password changes invalidate old sessions, global logout invalidates the recovery session, the old password fails, the new password works and used recovery proof is rejected. This direct verification used the provider's implicit recovery endpoint (AMR `otp`); the application's PKCE recovery uses AMR `recovery`. It does not establish inbox delivery or full emailed-link acceptance. [Provider evidence](validation/course-access/recovery-provider.json).
- The complete SQL migration chain ran against PostgreSQL 18. Separate tests preserved existing course/session content and nested reply classifications during migration. Real concurrent publishing/deletion tests in both orders, including a forced deadlock, preserved the invariant that a published resource cannot reference a deleting file. [Concurrency evidence](validation/course-access/postgres-concurrency.json).
- Both new migrations were applied to the intended Supabase project. The existing pilot retained its four dated sessions and gained exactly one general discussion. Course-owned materials use private authenticated delivery; the quota index covers course and owner lookups. Security advisors reported no new warning/error; the pre-existing leaked-password-protection warning remains unchanged.
- **38 real Supabase acceptance checks** through the local server verified admin/student separation, general and module discussions, filtered assignment replies, revision updates, private official uploads, exact file bytes, anonymous/direct Data API denial, saved intake/feedback and all four full CSV exports. Temporary accounts, files and course were removed. The storage-deletion check allows a bounded five-second origin visibility delay; the NODAL API must deny access immediately. [Predeployment evidence](validation/course-access/real-supabase-predeploy.json).
- Local browser checks verified conversation posting/replies, draft preservation across tabs and languages, staff file upload/save/download, existing feedback reports and a 390 px teaching layout without document overflow. The public homepage, its styles and main translation dictionary are unchanged.
- Following user feedback, the recovery page uses its own responsive stylesheet, a single step-specific heading, visible mobile language controls and a restrained urban-network illustration. Desktop (1365 px) and mobile (390 px) were visually reviewed; the change does not load or modify shared homepage styling.
